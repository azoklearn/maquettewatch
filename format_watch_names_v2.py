...
def update_catalogue():
    ...
    # Approche: trouver chaque watch-card, extraire la marque et le nom, puis remplacer
    # Pattern pour trouver chaque watch-card complet
    ...
    def replace_name(match):
        ...
        brand = match.group(3)
        full_name = match.group(4).strip()
        tag_end = match.group(5)
        
        # Nettoyer le nom s'il contient déjà "mm" à la fin
        if full_name.endswith('mm'):
            # Retirer le "mm" existant pour le reformater
            full_name = re.sub(r'\s+\d+mm\s*$', '', full_name)
